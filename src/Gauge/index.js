...
const Gauge = ({ value, min, max, label, colorStartScale, colorEndScale }) => {
  const percentScale = getPercentScale(min, max);
  const percent = percentScale(value);

  const angle = angleScale(percent);
  const filledArc = getFilledArc(angle);
  const colorScale = getColorScale(colorStartScale, colorEndScale);
...
Gauge.propTypes = {
  ...
  colorStartScale: PropTypes.string,
  colorEndScale: PropTypes.string,
};
